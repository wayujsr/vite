...
function animatebus(point, route, u_id, counter, steps, point_id, layers_list, sources_list) {
    ...
    if (counter < steps - 1) {
        ...
        console.log("Remove all layers and sources for cabs");
...
async function add_cab(origin, destination, u_id, counter) {
    var point_id = 'point_cab_' + u_id;
    var route_id = 'route_cab_' + u_id;
    var start_id = 'start_cab_' + u_id;
    var end_id = 'end_cab_' + u_id;
    ...
    map.addLayer({
        ...
        "type": "symbol",
        "layout": {
            "icon-image": CAB_ICON_IMAGE_NAME,
            // "icon-rotate": ["get", "bearing"],
            // "icon-rotation-alignment": "map",
            "icon-size": CAB_ICON_IMAGE_SIZE,
            "icon-allow-overlap": true,
            "icon-ignore-placement": true
        }
    });
...
async function add_all_cabs(){
    while (true) {
        // console.log(map_data.cab_data); 
        var cab_data_length = map_data.cab_data && map_data.cab_data.length;
        if (cab_data_length > 0) {
            var args = getCurrentSecondKey();
            // console.log("Searching cabs for: " + window.cab_counter);
            if (map_data.cab_data[window.cab_counter]) {
                var item = map_data.cab_data[window.cab_counter];
                // console.log("cabs found for: " + window.cab_counter + " with data: "+ item);
                for (var m = 0; m < item.features.length; m++) {
                    ...
                    for (var n = 0; n < mapping.features.length; n++) {
                        ...
                        var new_source = entities.source_city.geometry.sourcecabcoordinates;
                        var new_destination = entities.destination_city.geometry.destinationcabcoordinates;
                        var u_id = 'cab_' + window.cab_counter + '_' + m + '_' + n;
                            if (should_skip_cab(map) == true){
                                console.log("Due to overload, Skipping cabs with ID: " + u_id);
                                continue;
                            }
                        console.log("Add cab with ID: " + u_id);
                        add_cab(new_source, new_destination, u_id, 0);
                    }
                }
                window.cab_counter++
