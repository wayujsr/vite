...
async function add_all_charities() {
    while (true) {
        ...
        var charity_data_length = Object.keys(map_data.charity_data).length;
        if (charity_data_length > 0) {
            var args = getCurrentSecondKey();
            console.log("Searching charities for: " + args.start);
            if (map_data.charity_data.hasOwnProperty(args.start)) {
                var item = map_data.charity_data[args.start];
                console.log("Charities found for: " + args.start + " with data: "+ item);
